...
router.post('/sync',async(req,res,next)=>{
  ...
  try{
    ...
    for(const w of wallets){
      const r=await client.query('INSERT INTO wallets(user_id,name,wallet_type,opening_balance,is_active) VALUES($1,$2,$3,$4,$5) RETURNING id,name',[req.user.id,String(w.name).trim(),w.type||'other',Number(w.openingBalance)||0,w.isActive??true]);
      walletMap.set(String(w.id||w.name),r.rows[0].id); walletMap.set(String(w.name),r.rows[0].id);
    }
    const categoryMap=new Map();
    for(const c of categories){
      const r=await client.query('INSERT INTO categories(user_id,name,type) VALUES($1,$2,$3) RETURNING id,name',[req.user.id,String(c.name).trim(),c.type||'expense']);
      categoryMap.set(String(c.id||c.name),r.rows[0].id); categoryMap.set(String(c.name),r.rows[0].id);
    }
    for(const t of transactions){
      const walletId=walletMap.get(String(t.walletId||t.wallet||t.walletName));
      if(!walletId)continue;
      const categoryId=t.type==='transfer'?null:categoryMap.get(String(t.categoryId||t.category));
