...
router.get('/',async(req,res,next)=>{
  try{
    const result=await pool.query(`SELECT w.*,
      COALESCE(SUM(CASE WHEN t.type='income' AND t.wallet_id=w.id THEN t.amount
                        WHEN t.type='expense' AND t.wallet_id=w.id THEN -t.amount
                        WHEN t.type='transfer' AND tt.from_wallet_id=w.id THEN -t.amount
                        WHEN t.type='transfer' AND tt.to_wallet_id=w.id THEN t.amount ELSE 0 END),0) movement
      FROM wallets w LEFT JOIN transactions t ON t.user_id=w.user_id AND t.deleted_at IS NULL
      LEFT JOIN transaction_transfers tt ON tt.transaction_id=t.id
      WHERE w.user_id=$1 GROUP BY w.id ORDER BY w.created_at ASC`,[req.user.id]);
...
router.post('/',async(req,res,next)=>{
  ...
  try{
    const result=await pool.query('INSERT INTO wallets(user_id,name,wallet_type,opening_balance) VALUES($1,$2,$3,$4) RETURNING *',[req.user.id,name,walletType,openingBalance]);
