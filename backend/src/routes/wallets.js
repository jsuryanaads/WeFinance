...
router.get('/', async (req, res, next) => {
  try {
    const result = await pool.query(`
      ...
      FROM wallets w
      LEFT JOIN transactions t ON t.wallet_id=w.id AND t.user_id=$1 AND t.deleted_at IS NULL
      WHERE w.user_id=$1
      GROUP BY w.id ORDER BY w.created_at ASC`, [req.user.id]);
...
router.post('/', async (req, res, next) => {
  ...
  try {
    const result = await pool.query(`INSERT INTO wallets (user_id,name,wallet_type,opening_balance) VALUES ($1,$2,$3,$4) RETURNING *`, [req.user.id,name.trim(),walletType,openingBalance]);
